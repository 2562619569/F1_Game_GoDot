...
static func cfg(weather: String) -> Dictionary:
	return DATA.get(weather, DATA["sunny"])

## 表面摩擦修正（CarBuilder 应用到轮胎字典）
static func surface_mod(weather: String) -> Dictionary:
	var c := cfg(weather)
	return {"Road": c.road, "Dirt": c.dirt, "wet": c.wet}

## 生成比赛用 Environment
static func make_env(weather: String) -> Environment:
	var c := cfg(weather)
	...
	env.fog_density = 0.004 if weather != "sandstorm" else 0.012
	return env

static func setup_light(light: DirectionalLight3D, weather: String) -> void:
	var c := cfg(weather)
